...
import RefereeCreatePage from "./pages/RefereeCreatePage.jsx";
import RefereeCasePage from "./pages/RefereeCasePage.jsx";
import RefereeHistoryPage from "./pages/RefereeHistoryPage.jsx";
import ProtectedRoute from "./components/ProtectedRoute.jsx";
import { FileText, LayoutDashboard, Zap, LogIn, LogOut, User, Scale, History } from "lucide-react";

// App shell — dark Lucent / Placement Referee theme
function AppShell({ children }) {
  ...
  const navLinks = [
    { to: "/referee",         label: "Referee Case",   icon: Scale,           match: (p) => p === "/referee" },
    { to: "/referee/history", label: "Case History",   icon: History,         match: (p) => p === "/referee/history" },
    { to: "/upload",          label: "Legacy Screener", icon: FileText,        match: (p) => p === "/upload" || p.startsWith("/results") },
  ];
    ...
    <div className="lucent-shell grain flex flex-col min-h-screen">
      ...
      >
        ...
        >
          ...
          <span className="tracking-tight">PLACEMENT REFEREE</span>
...
export default function App() {
  return (
    <Routes>
      ...
      {/* Primary Placement Referee Routes */}
      <Route
        path="/referee"
        element={<ProtectedRoute><AppShell><RefereeCreatePage /></AppShell></ProtectedRoute>}
      />
      <Route
        path="/referee/history"
        element={<ProtectedRoute><AppShell><RefereeHistoryPage /></AppShell></ProtectedRoute>}
      />
      <Route
        path="/referee/:id"
        element={<ProtectedRoute><AppShell><RefereeCasePage /></AppShell></ProtectedRoute>}
      />

      {/* Legacy screening routes */}
      <Route
        path="/upload"
