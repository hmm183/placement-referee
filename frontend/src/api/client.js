...
// ── Placement Referee ─────────────────────────────────────────────────────────
export const createSoloCase = (formData, onUploadProgress) =>
  api.post("/api/referee/solo", formData, {
    headers: { "Content-Type": "multipart/form-data" },
    onUploadProgress,
    timeout: 300000, // 5 minutes for OCR + Gemini analysis
  });

export const addCandidateB = (caseId, formData, onUploadProgress) =>
  api.post(`/api/referee/${caseId}/candidate-b`, formData, {
    headers: { "Content-Type": "multipart/form-data" },
    onUploadProgress,
    timeout: 300000,
  });

export const getRefereeCase = (caseId) => api.get(`/api/referee/${caseId}`);

export const getRefereeCases = () => api.get("/api/referee");

export const deleteRefereeCase = (caseId) => api.delete(`/api/referee/${caseId}`);

export const fetchJobPostingUrl = (url) => api.post("/api/referee/fetch-url", { url });
